function PLSout = PLScorr_parallel(X, Y, opts)
%
% USAGE: PLSout = PLScorr_parallel(X, Y, opts)
...
% preallocate results for parallel loop
nLV = size(S,1);
Sp_perm = zeros(nLV, opts.nPerm);
explVarLVPerm = zeros(nLV, opts.nPerm);

parfor iter_perm = 1:opts.nPerm
    ...
    % calculate explained variance for each permutation run
    explVarLVPerm(:,iter_perm) = (Sp.^2) ./ sum(Sp.^2);
...
% determine dimensions for preallocation
nLV = size(U,2);
Ub_boots = zeros(size(U,1), nLV, opts.nBoot);
Vb_boots = zeros(size(V,1), nLV, opts.nBoot);
Sboot = zeros(nLV, opts.nBoot);
explVarLVsBoot = zeros(nLV, opts.nBoot);

parfor iter_boot = 1:opts.nBoot
    ...
    Ub = Ub * Sb_tmp * procmatU;
    Vb = Vb * Sb_tmp * procmatU;
    sb = sqrt(sum(Vb.^2));
    ...
    Sboot(:,iter_boot) = sb;
    explVarLVsBoot(:,iter_boot) = (sb.^2) ./ sum(sb.^2);
    Ub_boots(:,:,iter_boot) = Ub;
    Vb_boots(:,:,iter_boot) = Vb;
end

% compute standard error and bootstrap ratios
u_sum = sum(Ub_boots,3);
v_sum = sum(Vb_boots,3);
u_sqr = sum(Ub_boots.^2,3);
v_sqr = sum(Vb_boots.^2,3);
...
nFeatCI = min([size(Xb_in,2), size(Yb_in,2)]);
for k = 1:nFeatCI
